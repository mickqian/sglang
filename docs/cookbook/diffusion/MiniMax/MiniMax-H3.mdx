...
The linear adapter reuses SRT's GGUF type definitions, CUDA dequantization, and
low-token MMVQ dispatch. Normal DiT token matrices use dequantization followed
by the native GEMM because SRT's LLM-oriented MMQ path is slower at diffusion
sequence lengths. TP is supported when each row-parallel input shard remains
GGML-block aligned; incompatible degrees fail during model construction. FSDP,
LoRA merging, and the separate MiniMax-H3 AdaLN cache flags are not compatible
with packed GGUF weights.
