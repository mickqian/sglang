...
| DiT | ConvRot INT8, W4A8, W4A4, or mixed W4A4/W4A8 + INT8 safetensors ([INT8](https://huggingface.co/Comfy-Org/MiniMax-H3/tree/main/diffusion_models), [W4A8](https://huggingface.co/Winnougan/MiniMax-H3-INT4_Convrot_ComfyUI), [W4A4](https://huggingface.co/Merserk/MiniMax-H3-INT4-ConvRot), [mixed](https://huggingface.co/taxexempt/Custom-MiniMaX-H3-mixed-quants)) | `--component-weights-paths.transformer OWNER/REPO/path/FILE.safetensors` | Auto-detected per layer; requires `comfy-kitchen`. TP must preserve each file's ConvRot group boundaries. |
| DiT + text encoder | [TorchAO row-wise INT8 component bundle](https://huggingface.co/herb786/MiniMaxH3-acheze-int8) | `--component-paths.transformer herb786/MiniMaxH3-acheze-int8/transformer --component-paths.text_encoder herb786/MiniMaxH3-acheze-int8/text_encoder` | The component config declares the serialized format; SGLang safely flattens its tensor subclasses into the native row-wise INT8 runtime. Requires `torchao`. |
...
| Video VAE | [ConvRot INT8 safetensors](https://huggingface.co/GuangyuanSD/minimax_h3_video_vae_int8_convrot) | `--component-weights-paths.video_vae GuangyuanSD/minimax_h3_video_vae_int8_convrot/minimax_h3_video_vae_int8_convrot.safetensors` | Auto-detected native VAE path; requires `comfy-kitchen`. Add `--component-direct-gpu-weight-loading.video_vae` when the VAE stays resident. This is a capacity option: it lowers memory but can decode more slowly than BF16. |
...
| DiT or adapter | Timestep-distilled Turbo, as merged weights or LoRA ([Larry](https://huggingface.co/larryvrh/MiniMax-H3-Turbo-Lora), [LightX2V](https://huggingface.co/lightx2v/Minimax-h3-Turbo), [DaSiWa merged INT8](https://huggingface.co/brurpo/DaSiWa-MiniMax-H3-Hybrid), [Turbo + Remix LoRA](https://huggingface.co/t8star/minimax_h3_fl2v_turbo_8step_v1.0_768p_FeiHouRemix_v0.6_compat_v001_T8)) | Use `--component-weights-paths.transformer ...` for merged weights, or `--lora-path OWNER/REPO --lora-weight-name FILE --lora-merge-mode auto` for LoRA | Few-step semantic variant. Pin the exact FL2VA/Ref2VA file and its NFE/sigma schedule, scale, and alpha; storage-format detection does not infer sampling behavior. An 8-NFE release uses request value `num_inference_steps: 9`. See [LoRA recipes](#5-lora-recipes). |
...
| Adapter | [VR180 side-by-side LoRA](https://huggingface.co/rehan-fal/minimax-h3-vr180-sbs-lora) | `--lora-path rehan-fal/minimax-h3-vr180-sbs-lora --lora-weight-name h3-vr180-sbs-lora-v2.safetensors --lora-scale 1.0 --lora-merge-mode auto` | Standard PEFT adapter; use the published `vr180sbs` trigger and layout sentence with a 21:9 target. The generated frame contains both eye views; headset-ready rescaling and spherical metadata are post-processing, not a different H3 pipeline. |
| Adapter + execution contract | [H3-World action control](https://huggingface.co/DANNY621/H3-World) | `--model-variant fl2va --lora-path DANNY621/H3-World --lora-weight-name step-10000.safetensors --lora-merge-mode auto` | Use a first-frame `fl2va` request and pass `action_script`, with one instruction per resolved video latent (37 for 124 frames). SGLang applies the required directed CUDA FlexAttention routing; Ring parallelism is rejected. The LoRA file does not encode these semantics in metadata. |
...
H3 text-encoder sidecar key, not a standalone model component. Plain video/audio
VAE safetensors use `--component-weights-paths.video_vae` or
`--component-weights-paths.audio_vae`; the table lists the quantized VAE format
that has completed native end-to-end validation.
...
| Most aggressive speed preset (standard PEFT layout) | [`lightx2v/Minimax-h3-Turbo`](https://huggingface.co/lightx2v/Minimax-h3-Turbo), `minimax_h3_fl2v_turbo_4step_v1.2_768p_bf16.safetensors` | `num_inference_steps: 5` (4 denoiser evaluations), `flow_shift: 6.0`, `audio_flow_shift: 3.0`, `lora_scale: 1.0` | None |
...
The pinned v1.2 checkpoint declares alpha 8 in its metadata, so SGLang infers
it automatically; do not repeat it on the command line. Start with the Larry
8-evaluation recipe when preserving fine visual detail is more important than
minimum latency.
