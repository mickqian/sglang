...
Three things help, field-measured on that machine: `--pin-cpu-memory false`
(pinned memory cannot be reclaimed and starves the page cache the sweep lives
on — dropping it freed 14 GiB, raised read throughput 42% and cut the denoise
8.5%, at the price of a slower decode: a clear net win over a 50-step
request); resident DiT layers up to what VRAM truly holds (each keeps ~0.8 GB
per group off the disk — the flag counts per DiT group, and the 480P recipe
already sits near the 24 GB ceiling); and more RAM (64 GB caches the sweep and
returns to the quoted times — the only change that removes the disk from the
loop rather than negotiating with it). The
