...
- `speed` keeps model components resident, `auto` applies the model-aware 120 GiB residency threshold, and `memory` enables the memory-saving placement policy. VAEs stay resident in every implicit placement; explicit `--vae-cpu-offload` or `--layerwise-offload-components ... vae` remains available for a measured low-memory recipe such as the 2×RTX 5090 configuration above. DiT residency/prefetch knobs are scoped to the DiT. When `memory` is combined with explicit FSDP, H3 keeps the sharded DiT and VAE on GPU and layerwise-offloads the text encoder. Use `speed` only after confirming that the complete target workload fits.
