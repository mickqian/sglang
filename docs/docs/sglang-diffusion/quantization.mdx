...
Packed linears reuse SRT's GGUF type definitions, CUDA dequantization, and
low-token MMVQ dispatch. Normal diffusion token matrices use dequantization
followed by the native GEMM because the LLM-oriented MMQ path is slower at
diffusion sequence lengths. GGUF remains a capacity-oriented option; latency
