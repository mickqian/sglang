...
- `--component-paths.<component>`: replace any component declared in
  `model_index.json` with a complete local or Hub component repo
...
Components without an SGLang-specific loader delegate serialized
`quantization_config` admission and restoration to the installed Transformers
or Diffusers AutoQuantizer. This covers the formats that the selected upstream
library and component architecture actually support; it is not a blanket
promise that every format runs on every device. SGLang-specific text/image
encoders, DiTs, and VAEs retain explicit capability checks and fail closed
...
Other encoder architectures remain fail-closed until their loader and output
contract are admitted explicitly.
