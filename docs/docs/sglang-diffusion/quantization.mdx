...
| Native text/image encoder | Uses an explicit architecture capability: H3 Qwen3-VL FP8, T5/UMT5/CLIP text BnB4, and CLIP vision BnB4 or serialized SRT FP8 are currently admitted. |
...
`--image-encoder-precision`. Other native encoder architectures remain
fail-closed until their loader and output contract are admitted explicitly.
