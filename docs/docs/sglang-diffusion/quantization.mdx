...
tag: "approx"
metatags:
    description: "Configure transformer, encoder, VAE, and other component checkpoint quantization, plus Quant-VideoGen causal KV-cache quantization in SGLang-Diffusion."
---

SGLang-Diffusion resolves quantization independently for each weight-bearing
pipeline component. A quantized DiT does not implicitly quantize the text or
image encoder, VAE, or any other component.
...
- `--component-paths.<component>`: replace any component declared in
  `model_index.json` with a complete local directory or Hub component repo;
  `--<component>-path` is the shorter alias
- `--component-weights.<component>`: replace weights while retaining the base
  component config; only use this when that component loader documents its
  accepted standalone weight format
...
`--component-paths.<component>` is the canonical day-0 interface. The selected
directory or Hub repo must be a self-contained component root with its own
`config.json` and weights. SGLang reads its serialized `quantization_config`;
no separate quantization flag is required.

| Component loader | Serialized checkpoint handling |
| --- | --- |
| No SGLang-specific loader | Delegates admission and restoration to the installed Transformers or Diffusers AutoQuantizer. |
| Specialized loader without a quantization implementation | Bypasses the ordinary SGLang weight path and delegates to the upstream AutoQuantizer. If the upstream architecture or format is unsupported, loading fails explicitly. |
| Native DiT | Uses the formats and platform constraints listed in the tables below. |
| Native text/image encoder | Uses an explicit optimized capability when available: H3 Qwen3-VL FP8, T5/UMT5/CLIP text BnB4, and CLIP vision BnB4 or serialized SRT FP8 are currently admitted. Otherwise, non-native-only pipelines delegate formats accepted by Transformers. |
| VAE | A non-native VAE may use a top-level Diffusers `quantization_config`; native-only VAEs reject formats they do not implement. |

This reuses the upstream format ecosystem where possible, but it is not a
blanket promise that every format runs on every architecture, topology, or
device. Unsupported combinations fail closed rather than silently loading a
different precision. Use `--artifact-preflight metadata` to inspect artifact
selection and serialized metadata before allocating a GPU.
...
## Encoder component quantization

T5 and UMT5 text encoders can load serialized BitsAndBytes 4-bit checkpoints
through Transformers. For example, replace FLUX's second text encoder with the
official Diffusers component checkpoint:
...
The quantized encoder must stay resident. SGLang rejects component or layerwise
offload for this path instead of silently dequantizing or changing its state.
Serialized BitsAndBytes 4-bit `CLIPTextModel` and `CLIPVisionModel` components
use the same contract through `--component-paths.text_encoder`,
`--component-paths.text_encoder_2`, or `--component-paths.image_encoder`.
Native `CLIPVisionModel` image encoders also accept serialized FP8 through the
shared SRT FP8 linear backend. The component must remain resident; its
activation precision remains controlled independently by
`--image-encoder-precision`. Other encoder architectures delegate serialized
formats accepted by Transformers when the pipeline permits native-library
fallback. A pipeline that requires an SGLang-native encoder, such as H3's
layer-50 Qwen3-VL extractor, still fails closed unless that exact native
architecture/format pair is admitted.
