...
Serialized BitsAndBytes 4-bit `CLIPVisionModel` image encoders use the same
contract through `--component-paths.image_encoder`; other image-encoder
architectures remain fail-closed until their loader and output contract are
admitted explicitly.
