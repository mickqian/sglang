...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><code>--transformer-weights-path</code></td>
      <td>MiniMax-H3 (validated, non-pruned <code>fl2va</code>)</td>
      <td>None</td>
      <td>CUDA only, <code>--tp-size 1</code>, no FSDP. Shrinks the download and the host memory offload pins (17.5 vs 61.7 GiB for H3) rather than peak VRAM, which offload already bounds. Dequantized per use, so it is not faster. See <a href="#gguf">GGUF</a>.</td>
    </tr>
  </tbody>
</table>
...
Packed linears reuse the SRT GGUF implementation. Standard and K-quants run
through its MMVQ/MMQ kernels without first materializing a dense weight. Types
without an MMQ kernel use SRT's existing dequantize-and-matmul fallback for
larger activation batches. GGUF remains a capacity-oriented option; latency
depends on the quantization type, activation shape, and placement policy.
...
| CUDA only | The reused SRT GGML matmul kernels currently ship for CUDA |
...
Not validated in the measurement above: any other quantization type, the
`ref2va` partition, multi-GPU, and a BF16-vs-GGUF output comparison.
