...
`ss_network_alpha`) is reported. A startup LoRA can provide those request
defaults, but manifest defaults take precedence and explicit request values
always win; filename-only guesses never change runtime behavior.
