...
Diffusers `auto_map` classes, tied state entries, and any component-offload or
layerwise-offload placement. Self-describing ConvRot INT8 VAE checkpoints use
the same path when every marked linear matches the native component graph.
