...
- PEFT compatibility: standard LoRA and RSLoRA adapters are accepted. RSLoRA scaling is folded into the loaded B weights, preserving its `alpha / sqrt(rank)` result; `alpha_pattern` becomes the native per-layer `.alpha` contract. DoRA currently fails closed instead of being silently interpreted as ordinary LoRA.
