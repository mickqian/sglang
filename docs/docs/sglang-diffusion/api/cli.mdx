...
- PEFT compatibility: standard LoRA and RSLoRA adapters are accepted. RSLoRA scaling is folded into the loaded B weights, preserving its `alpha / sqrt(rank)` result; `alpha_pattern` becomes the native per-layer `.alpha` contract. DoRA and adapters that require auxiliary saved modules or runtime routing currently fail closed instead of being silently interpreted as ordinary LoRA.
...
- `--component-weights.<component>` for a weights-only component override that
  retains the base pipeline's component config. `--transformer-weights-path` is
  the compatibility alias for `--component-weights.transformer`; other component
  types fail explicitly until their loader supports that checkpoint format.
...
never silently changes prompt embeddings or VAE outputs:

- `--component-paths.<component> {MODEL}` replaces any `model_index.json`
  component with a self-contained local directory or Hub component repo;
  `--<component>-path` is its shorter alias.
- Examples include `--component-paths.text_encoder`,
  `--component-paths.image_encoder`, `--component-paths.vae`, and
  `--component-paths.transformer`.
- Serialized quantization metadata is auto-detected. Generic components reuse
  the installed Transformers/Diffusers AutoQuantizer; native SGLang components
  use an optimized architecture/format capability when available and otherwise
  delegate to the upstream loader when the pipeline permits it. Native-only or
  unsupported combinations fail before ordinary weight loading instead of
  silently changing precision.

### Artifact preflight

Artifact publishers can package stable defaults in a non-executable
`sglang_artifacts.json` and users can apply it with `--artifact-manifest`:
...
Paths are relative to the manifest. Supported default roles are `component`,
`component_weights`, and `lora`; arbitrary backend names, key mappings, and
executable code are not accepted. Explicit CLI or request values always win.
Manifest request defaults must name fields on the selected model's concrete
`SamplingParams` class. Optional SHA-256 checksums apply to exact files and are
verified after download, before the artifact is loaded.

Entries without `"default": true` remain available as named release variants.
Select one with `--artifact-entry <id>`; repeat the option for independent
components. An explicitly selected entry replaces the manifest default for the
same component or LoRA slot, while direct `--component-paths.*`,
`--component-weights.*`, and `--lora-path` arguments still take precedence:

```bash Command
sglang serve \
  --artifact-manifest owner/release-package \
  --artifact-entry distilled \
  --artifact-entry fp8-encoder
```

An entry may include its own `request_defaults` for variant-specific steps,
guidance, or flow shift. Selected-entry defaults override default-entry values,
which override root manifest defaults; conflicting active entries at the same
priority are rejected.

Use the normal `generate` or `serve` command to inspect every configured model,
component override, weights-only override, and startup LoRA before allocating a
GPU:
...
Explicit numeric safetensors metadata for sampling steps, guidance scale, flow
shift, and LoRA alpha (`alpha`, `lora_alpha`, `network_alpha`, or
`ss_network_alpha`) is reported. For a PEFT adapter, the sibling
`adapter_config.json` takes precedence for alpha and identifies ordinary LoRA,
RSLoRA, or DoRA. A startup LoRA can provide those request defaults, but manifest
defaults take precedence and explicit request values always win; filename-only
guesses never change runtime behavior.
