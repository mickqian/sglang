...
- `--direct-gpu-weight-loading {true|false}`: opt into direct GPU loading for TransformerLoader components. Use `--component-direct-gpu-weight-loading.<exact-model-index-key> {true|false}` (or the equivalent YAML boolean map) to select individual native TransformerLoader components or `dual_tower_bridge`; an exact entry overrides the legacy global flag. Selected components must use unquantized safetensors on CUDA, stay GPU-resident, use TP=1, and not use FSDP. Compatible tensors become parameter storage without another GPU copy, but transformed tensors can still need temporary allocations, so startup time and peak GPU memory remain model-dependent. Other component loaders and the Diffusers backend fail closed.
