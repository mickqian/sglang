...
- PEFT compatibility: standard LoRA and RSLoRA adapters are accepted. RSLoRA scaling is folded into the loaded B weights, preserving its `alpha / sqrt(rank)` result. DoRA and `alpha_pattern` currently fail closed instead of being silently interpreted as ordinary LoRA; export ordinary LoRA weights or explicit per-layer `.alpha` tensors for now.
