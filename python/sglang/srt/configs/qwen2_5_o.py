...
import numpy as np
from transformers import PretrainedConfig, Qwen2VLImageProcessor
...
class Qwen2_5OmniProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        super().__init__(
            omni_processor, feature_extractor, tokenizer, chat_template=chat_template
        )
    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2_5OmniProcessorKwargs],
        ...
        if audios is not None:
            ...
            input_lengths = (
                audios_inputs["feature_attention_mask"].sum(-1).numpy() - 1
            ) // 2 + 1
        ...
        if images is not None:
            images_inputs = self.omni_processor(
        ...
        if videos is not None:
            videos_inputs = self.omni_processor(
                images=None, videos=videos, **output_kwargs["videos_kwargs"]
            )
            if fps is None:
                fps = [2.0] * len(videos)
            videos_inputs["video_second_per_grid"] = [
                self.omni_processor.temporal_patch_size / fps[i]
                    ...
                    if use_audio_in_video:
                        ...
                        video_t_index = (
                            ...
                            .expand(
                                ...
                                // self.omni_processor.merge_size,
        ...
        print(f"{audios_inputs=}")
        print(f"{videos_inputs=}")
        print(f"{images_inputs=}")
        if "input_features" in audios_inputs:
            print("111")
            print(audios_inputs["input_features"].shape)
        return BatchFeature(
...
register_image_processor(Qwen2_5OmniConfig, Qwen2VLImageProcessor)
