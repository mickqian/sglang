...
from sglang.srt.layers.attention.vision import MMAttention
...
class MllamaForConditionalGeneration(nn.Module):
    ...
    def pad_input_ids(self, input_ids: List[int], mm_inputs: MultimodalInputs):
        pixel_values = torch.cat([item.pixel_values for item in mm_inputs.items], dim=0)
        ...
        for i, mm_input in enumerate(forward_batch.mm_inputs):
            ...
            if not forward_batch.encoder_cached[i] and mm_input is not None:
                pixel_values = torch.cat(
                    [item.pixel_values for item in mm_input.items], dim=0
                )
        ...
        with forward_batch.out_cache_loc.device:
            ...
            for k, mm_input in enumerate(forward_batch.mm_inputs):
                ...
                for j in range(pixel_values.shape[1]):
                    ...
                    batched_ar_ids[i, j] = mm_input.items[0].aspect_ratio_id[0, j]

                    batched_ar_mask[i, j, :num_tiles] = mm_input.items[
                        0
