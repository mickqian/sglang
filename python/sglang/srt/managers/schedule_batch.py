...
class MultimodalDataItem:
    ...
    modality: Literal["image", "multi-images", "video", "audio"]
    ...
    def is_audio(self):
        return (self.modality == "audio") and not MultimodalDataItem.is_empty_list(
            self.audio_features
        )

    def is_image(self):
        return (
            self.modality == "image" or self.modality == "multi-images"
        ) and not MultimodalDataItem.is_empty_list(self.pixel_values)

    def is_video(self):
        return (self.modality == "video") and not MultimodalDataItem.is_empty_list(
            self.pixel_values
        )

    def validate(self):
        ...
        # assert isinstance(self.pixel_values, torch.Tensor)
        # assert self.audio_features is None or isinstance(ret.audio_features, list)
        #
...
class MultimodalInputs:
    ...
    def contains_image_inputs(self) -> bool:
        """ """
        return any(item.is_image() for item in self.items)

    def contains_audio_inputs(self) -> bool:
        """ """
        return any(item.is_audio() for item in self.items)

    def collect_image_inputs(self) -> List[torch.Tensor]:
        return [item.pixel_values for item in self.items if item.is_image()]
