...
def make_mistral_chat_completion_request(
    ...
    # convert to the format as in mistral_common
    converted_messages = []
    for message in messages:
        ...
        # 其他角色类似处理
    print(f"messages: {converted_messages}")

    return ChatCompletionRequest(messages=converted_messages, tools=tools)
...
class MistralTokenizer(TokenizerBase):
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        print(f"mistral request: {request}")
    ...
    def batch_decode(
        ...
        # TODO: check spaces_between_special_tokens
        return [
            self.decode(
                seq,
    ...
    def convert_ids_to_tokens(
        ...
        if self.is_tekken:
            ...
            ids = [
                i
                for i in ids
                if i > self.tokenizer.num_special_tokens
                or i == self.tokenizer.get_control_token(SpecialTokens.tool_calls)
            ]
