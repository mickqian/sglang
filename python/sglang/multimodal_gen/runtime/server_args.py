    ...
    def _adjust_attention_backend(self):
        ...
        if self.attention_backend is None and self.backend != Backend.DIFFUSERS:
            if (
                ...
                and self.pipeline_class_name
                in (None, "LTX2Pipeline", "LTX2TwoStagePipeline")
            ):
                self.attention_backend = "torch_sdpa"
                logger.info(
                    "Automatically set attention_backend=torch_sdpa for LTX-2.3 single-GPU native pipeline to preserve precision"
                )
                return
