...
def _rmsnorm_eps(norm: nn.Module) -> Optional[float]:
    eps = getattr(norm, "variance_epsilon", None)
    return eps if eps is not None else getattr(norm, "eps", None)


def _rmsnorm_weight(norm: nn.Module) -> Optional[torch.Tensor]:
...
def apply_qk_norm_across_heads(
    ...
    q_weight = _rmsnorm_weight(q_norm)
    k_weight = _rmsnorm_weight(k_norm)
    q_eps = _rmsnorm_eps(q_norm)
    k_eps = _rmsnorm_eps(k_norm)
    ...
    if (
        ...
        and getattr(q_norm, "full_hidden_size", hidden_size) == hidden_size
        and getattr(k_norm, "full_hidden_size", hidden_size) == hidden_size
        and q.shape == k.shape
