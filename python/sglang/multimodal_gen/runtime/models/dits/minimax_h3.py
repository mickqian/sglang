...
def _pdd_plan_from_sigmas(
    ...
    *,
    ...
    if not torch.isfinite(schedule).all() or not torch.all(schedule[:-1] > schedule[1:]):
...
class MiniMaxH3DiTModel(BaseDiT, LayerwiseOffloadableModuleMixin):
    ...
    def split_lora_runtime_state(
        ...
        def shard(
            ...
            *,
            ...
            expected = (
                (num_steps, layer.output_size)
                if is_bias
                else (num_steps, layer.output_size, self.arch.hidden_size)
            )
            ...
            start = layer.tp_rank * layer.output_size_per_partition
            return tensor.narrow(1, start, layer.output_size_per_partition).to(
                device=self.device,
                copy=True,
            )
    ...
    def set_lora_runtime_state(
        ...
        if strength != 1.0:
            raise ValueError(
                "MiniMax H3 PDD output heads are absolute weights; use "
                "lora_scale=1.0"
            )
