...
from sglang.multimodal_gen.runtime.layers.layernorm import RMSNorm as SGLRMSNorm
...
def ltx2_scale_shift(
    ...
    if _ltx2_can_use_fused_scale_shift(x, shift, scale):
        return fuse_scale_shift_kernel(x, scale, shift)
...
def ltx2_residual_rms_norm_scale_shift(
    ...
    if (
        _ltx2_can_use_fused_scale_shift(residual, shift, scale)
        and x.is_cuda
        and x.shape == residual.shape
        and x.is_contiguous()
        and gate.is_cuda
...
class LTX2Attention(nn.Module):
    def __init__(
        ...
        if self.qk_norm:
            if tp_size == 1:
                self.q_norm = SGLRMSNorm(self.inner_dim, eps=self.norm_eps)
                self.k_norm = SGLRMSNorm(self.inner_dim, eps=self.norm_eps)
            else:
...
class LTX2TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.cross_attention_adaln:
            norm_hidden_states, hidden_states = ltx2_residual_rms_norm_scale_shift(
                hidden_states,
                attn_hidden_states,
                vgate_msa,
                self.norm_eps,
        ...
        if self.cross_attention_adaln:
            (
                norm_audio_hidden_states,
                audio_hidden_states,
            ) = ltx2_residual_rms_norm_scale_shift(
                audio_hidden_states,
        ...
        if self.cross_attention_adaln:
            ...
            mod_encoder_hidden_states = ltx2_scale_shift(
            ...
            mod_audio_encoder_hidden_states = ltx2_scale_shift(
        ...
        # A2V
        mod_norm_hidden_states = ltx2_scale_shift(
            norm_hidden_states, video_a2v_ca_shift, video_a2v_ca_scale
        )
        mod_norm_audio_hidden_states = ltx2_scale_shift(
        ...
        if not skip_a2v_cross_attn:
            ...
            ff_norm_hidden_states, hidden_states = ltx2_residual_rms_norm_scale_shift(
                hidden_states,
                a2v_attn_hidden_states,
                a2v_gate,
                self.norm_eps,
            ...
            ff_norm_hidden_states = ltx2_rms_norm_scale_shift(
        ...
        if not skip_v2a_cross_attn:
            ...
            ff_norm_audio_hidden_states = ltx2_rms_norm_scale_shift(
