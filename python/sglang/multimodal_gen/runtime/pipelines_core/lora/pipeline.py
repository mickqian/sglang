...
class LoRAPipeline(ComposedPipelineBase):
    ...
    def _apply_parameter_deltas(
        ...
        *,
        ...
        deltas: defaultdict[str, list[tuple[torch.Tensor, float]]] = defaultdict(list)
    ...
    def _set_lora_runtime_state(
        ...
        if isinstance(module, BaseDiT):
            ...
            raise TypeError(f"{type(module).__name__} cannot apply LoRA runtime state")
    ...
    def deactivate_lora_weights(self, target: str = "all") -> None:
        ...
        for module_name, lora_layers_dict in target_modules:
            if (
                self.is_lora_merged.get(module_name, False)
                or any(layer.merged for layer in lora_layers_dict.values())
                or module_name in self.active_parameter_delta_modules
            ):
    ...
    def unmerge_lora_weights(self, target: str = "all") -> None:
        ...
        for module_name, lora_layers_dict in target_modules:
            if not self.is_lora_merged.get(module_name, False):
                if (
                    self._has_active_unmerged_lora(lora_layers_dict)
                    or module_name in self.active_parameter_delta_modules
                ):
                    self._apply_parameter_deltas(module_name, [], [])
                    self._set_lora_runtime_state(module_name, [], [])
