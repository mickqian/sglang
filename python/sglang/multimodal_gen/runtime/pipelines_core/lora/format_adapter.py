...
def _normalize_additive_patches(
    ...
    for base, delta in weight_patches.items():
        ...
        if delta.ndim != 2 or min(delta.shape) == 0:
            raise ValueError(
                ...
                "non-empty linear weight or parameter vector, got shape "
                f"{tuple(delta.shape)}"
            )
    ...
    for base in sorted(bias_patches.keys() - weight_patches.keys()):
        ...
        if b_key not in normalized:
            raise ValueError(
                "Additive bias patch has neither a matching linear .diff nor "
                f"LoRA projection (e.g. {base + _LINEAR_DIFF_BIAS_SUFFIX!r})"
            )
