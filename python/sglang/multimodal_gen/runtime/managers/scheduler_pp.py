...
from sglang.multimodal_gen.runtime.distributed.disagg_communicators.base_communicator import DisaggCommunicator
...
class SchedulerPPMixin:
    ...
    def run_batch(self: "Scheduler", comm: DisaggCommunicator, req_to_run: Req, req_identity):
        try:
            # Execute
    ...
    def event_loop_pp(self: "Scheduler") -> None:
        ...
        while self._running:
            ...
            try:
                ...
                logger.error(f"Error receiving requests from client: {e}", exc_info=True)
                new_reqs = []
    ...
    def _post_recv_size(self, comm: DisaggCommunicator):
        ...
        try:
            ...
            if comm.is_non_dit_rank():
                # Listen to DiT
                _, works = comm.batch_irecv_from_dit(
                    [(self.recv_size_tensor, torch.long)]
                )
            else:
                # Listen to Non-DiT
                _, works = comm.batch_irecv_from_non_dit(
                    [(self.recv_size_tensor, torch.long)]
                )
    ...
    def _process_pipeline_recv(self, comm: DisaggCommunicator):
        """Check if pipeline data arrived and process it."""
        ...
        if self.recv_size_work.is_completed():
            ...
            try:
                ...
                # 1. Recv Metadata
                meta_tensor = torch.empty((size,), dtype=torch.uint8, device=self.recv_size_tensor.device)
                if comm.is_non_dit_rank():
                    _, w = comm.batch_irecv_from_dit([(meta_tensor, torch.uint8)])
                else:
                    _, w = comm.batch_irecv_from_non_dit([(meta_tensor, torch.uint8)])
                ...
                if all_shapes:
                    ...
                    for info in tensor_names:
                        ...
                        if info[0] == "single":
                            ...
                            name, idx, length, indices = info[1], info[2], info[3], info[4]
                            if name not in list_buffers:
                                list_buffers[name] = {"length": length, "indices": indices, "tensors": []}
    ...
    def _handle_success(self: "Scheduler", req: Req, output_batch: OutputBatch, identity, comm: DisaggCommunicator):
    ...
    def _broadcast_batch_in_dit_group(self, batch, comm: DisaggCommunicator):
        ...
        if dist.get_rank() == comm.dit_master_rank:
            pkg = (batch._recv_metadata, batch._recv_tensor_infos, batch._recv_list_tensor_infos)
        else:
            pkg = None
        ...
        # 1. Broadcast Meta
        pkg = broadcast_pyobj(pkg, dist.get_rank(), group, comm.dit_master_rank,
                              force_cpu_device=(dist.get_backend(group) != "nccl"))

        if dist.get_rank() != comm.dit_master_rank:
            meta, t_infos, l_infos = pkg
            for k, v in meta.items(): setattr(batch, k, v)
        ...
        for k, v in batch.__dict__.items():
            ...
            if isinstance(v, torch.Tensor):
                if not v.is_cuda: v = v.cuda()
                comm.broadcast_in_group(v)
            elif isinstance(v, list):
                for item in v:
                    if isinstance(item, torch.Tensor):
                        if not item.is_cuda: item = item.cuda()
    ...
    def _pack_tensors(self, metadata, t_infos, l_infos, tensors):
        ...
        meta_bytes = torch.tensor(bytearray(buf.getvalue()), dtype=torch.uint8, device="cpu")
        if dist.get_backend() == "nccl":
            meta_bytes = meta_bytes.cuda()

        size_tensor = torch.tensor([meta_bytes.numel()], dtype=torch.long, device=meta_bytes.device)
        all_tensors = [size_tensor, meta_bytes]

        for name in t_infos:
            t = tensors[name]
            if not t.is_cuda: t = t.cuda()
