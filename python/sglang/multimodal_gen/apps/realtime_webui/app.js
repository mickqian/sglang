...
const examplePresets = [
  ...
  { name: "Plastic Beach Orbit", tone: "blue", size: "832x480", fps: 25, prompt: "An automatic extremely slow clockwise orbit around the Plastic Beach island from the reference image: the island remains centered above the dark midnight-blue ocean, the white lighthouse and its reflection path stay recognizable, the starry navy sky and deep blue palette stay consistent, and the camera keeps a constant low altitude and constant distance while rotating only a few degrees across the whole clip. Preserve the original island silhouette, horizon, waterline, and album-cover composition; no push-in, no camera descent, no fast turn, no turquoise color shift, no morphing of the lighthouse or island structures.", referenceUrl: "https://is1-ssl.mzstatic.com/image/thumb/Music/v4/b8/f9/b9/b8f9b9f8-a609-bde2-0302-349436ffc508/825646291038.jpg/600x600bb.jpg", source: "Gorillaz Plastic Beach artwork", mime: "image/jpeg" },
...
function recordPostprocessedFrameBatch(decodedFrames) {
  if (!recordingActive || recordingSaving) return;
  const duration = recordingFrameDurationUs();
  for (const item of decodedFrames) {
    ...
    recordFrame(item.image, timestamp, duration);
  }
  updateRecordButton();
}

function recordingFrameDurationUs() {
...
async function decodeAndEnqueueFrameBatch(header, data, epoch) {
  ...
  const now = performance.now();
  recordPostprocessedFrameBatch(decodedFrames);
