...
class TestSerializedVAEQuantization(unittest.TestCase):
    ...
    def _load_checkpoint(self, checkpoint: pathlib.Path, direct_gpu_loading: bool):
        ...
        server_args.component_direct_gpu_weight_loading = {"vae": direct_gpu_loading}
        with (
            patch.object(
                vae_loader,
                "get_diffusers_component_config",
        ...
        with TemporaryDirectory() as root:
            ...
            for direct_gpu_loading in (False, True):
                with self.subTest(direct_gpu_loading=direct_gpu_loading):
                    ...
                    self.assertEqual(loaded.decoder.proj.weight.device.type, "cpu")
    ...
    def test_native_vae_restores_plain_fp8_storage(self):
        with TemporaryDirectory() as root:
            ...
            safetensors_save_file(
                {
                    "decoder.proj.weight": torch.arange(64, dtype=torch.float32)
                    .reshape(4, 16)
            ...
            for direct_gpu_loading in (False, True):
                with self.subTest(direct_gpu_loading=direct_gpu_loading):
                    ...
                    self.assertTrue(
                        torch.equal(loaded.decoder.proj.weight_scale, torch.ones(1))
                    )
                    self.assertNotIn("decoder.proj.weight_scale", loaded.state_dict())
