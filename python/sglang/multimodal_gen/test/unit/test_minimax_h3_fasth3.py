...
def test_h3_pdd_head_bank_is_split_from_plain_lora_and_tp_sharded() -> None:
    video = SimpleNamespace(output_size=4, output_size_per_partition=2, tp_rank=1)
    audio = SimpleNamespace(output_size=2, output_size_per_partition=1, tp_rank=1)
    model = SimpleNamespace(
    ...
    torch.testing.assert_close(state["audio_bias"], adapter["audio_proj_out.bias"][:, 1:])
...
def test_h3_pdd_plan_matches_released_shifted_fine_grid() -> None:
    ...
    assert all(
        float(coefficients.sum()) == pytest.approx(1.0)
        for _, coefficients in plans
    )
