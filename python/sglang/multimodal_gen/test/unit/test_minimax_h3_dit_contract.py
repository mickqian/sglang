...
def test_final_heads_keep_checkpoint_declared_packed_layout():
    ...
    class FinalHeadOnlyGGUFConfig(GGUFConfig):
        def get_quant_method(self, layer, prefix):
            if self.has_packed_weight(prefix):
    ...
    with torch.device("meta"):
        model = MiniMaxH3DiTModel(
            ...
            hf_config={},
            quant_config=FinalHeadOnlyGGUFConfig("minimax-h3.gguf", tensor_meta),
        )
    ...
    native_weights = dict(
        _diffusers_h3_checkpoint(
            [
                ...
                ("condition_proj.weight_scale", native_scale),
            ],
            valid_target_names={
                "blocks.39.attn.out_proj.weight_scale",
                "condition_proj.weight_scale",
            },
        )
    )
    assert set(native_weights) == {
        "blocks.39.attn.out_proj.weight_scale",
        "condition_proj.weight_scale",
    }
    assert torch.equal(
