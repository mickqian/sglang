...
def test_overlay_model_index_resolution_and_materialization(monkeypatch, tmp_path):
    ...
    assert all("safetensors" not in pattern for pattern in snapshot_calls[0]["allow_patterns"])
    ...
    assert (materialized_path / "transformer" / "diffusion_pytorch_model.safetensors").read_text(
        encoding="utf-8"
    ) == "transformer-weights"
    assert (materialized_path / "vae" / "diffusion_pytorch_model.safetensors").read_text(
        encoding="utf-8"
    ) == "vae-weights"
    assert (materialized_path / ".sglang_overlay_materialized.json").exists()

    second_dir = hf_utils.maybe_download_model(source_model_id, force_diffusers_model=True)
    assert second_dir == materialized_dir

    source_calls = [call for call in snapshot_calls if call["repo_id"] == source_model_id]
    overlay_calls = [call for call in snapshot_calls if call["repo_id"] == overlay_repo_id]
        ...
        if repo_id == overlay_repo_id:
            if kwargs.get("allow_patterns") is not None:
                ...
                _write_json(
                    ...
                    json.loads((overlay_repo / "model_index.json").read_text(encoding="utf-8")),
                )
                _write_json(
...
def test_overlay_custom_materializer(monkeypatch, tmp_path):
    ...
    assert (materialized_path / "model_index.json").exists()
    assert (materialized_path / "transformer" / "config.json").exists()
    assert (materialized_path / "transformer" / "official_code" / "wan" / "__init__.py").exists()
    assert (materialized_path / "transformer" / "checkpoints" / "weights.bin").read_text(
        encoding="utf-8"
    ) == "fake-weights"
