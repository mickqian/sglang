...
"""Device staging for per-module post-load weight processing."""
...
def _stage_data_to_device(data: torch.Tensor, device: torch.device) -> torch.Tensor:
    return data.to(device)
...
def stage_module_for_post_load(
    module: nn.Module,
    process_device: torch.device | str,
    *,
    ...
    process_device = torch.device(process_device)
    ...
    original_owners: dict[int, nn.Module] = {}
    ...
    for owner, registry_name, name, tensor in _iter_registered_tensors(module):
        ...
        # Keep ids stable if a hook replaces a child module.
        original_owners[id(owner)] = owner
    ...
    try:
        for state in tensor_states.values():
            if state.origin != process_device:
                state.staged_data = _stage_data_to_device(
                    state.tensor.data, process_device
                )
